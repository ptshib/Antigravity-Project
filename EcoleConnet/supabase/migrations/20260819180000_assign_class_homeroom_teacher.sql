-- ============================================================================
-- Migration : Gestion Sécurisée et Atomique du Professeur Titulaire de Classe
-- Fichier : supabase/migrations/20260819180000_assign_class_homeroom_teacher.sql
-- ============================================================================

BEGIN;

--------------------------------------------------------------------------------
-- 1. INDEX UNIQUE PARTIEL : UNE SEULE CLASSE PAR TITULAIRE ET PAR ANNÉE
--------------------------------------------------------------------------------

CREATE UNIQUE INDEX IF NOT EXISTS uq_classes_active_homeroom_teacher
  ON public.classes (school_id, academic_year_id, homeroom_teacher_id)
  WHERE homeroom_teacher_id IS NOT NULL;

--------------------------------------------------------------------------------
-- 2. RPC DÉDIÉE : assign_class_homeroom_teacher
--------------------------------------------------------------------------------
...
BEGIN
  ...
  -- F. Contrôle Rigoureux de l'Enseignant Cible & son Profil
  SELECT t.id, t.school_id, t.profile_id, t.employee_number, t.specialty, t.account_status, t.employment_status,
...
--------------------------------------------------------------------------------
-- 3. RPC DÉDIÉE : remove_class_homeroom_teacher
--------------------------------------------------------------------------------
...
BEGIN
  ...
  -- E. Infos sur l'Ancien Titulaire
  SELECT t.id, t.employee_number, p.first_name, p.last_name
  INTO v_old_teacher
  FROM public.teachers t
  JOIN public.profiles p ON p.id = t.profile_id
  ...
  -- G. Journalisation Audit Sécurisée (Structure Réelle de public.school_audit_logs)
  INSERT INTO public.school_audit_logs (
    school_id,
    actor_id,
    action,
...
1. Index unique partiel :
   - public.uq_classes_active_homeroom_teacher sur (school_id, academic_year_id, homeroom_teacher_id) WHERE homeroom_teacher_id IS NOT NULL.
2. Fonctions RPC sécurisées :
   - public.assign_class_homeroom_teacher(p_class_id UUID, p_teacher_id UUID, p_expected_current_teacher_id UUID DEFAULT NULL) RETURNS JSONB.
   - public.remove_class_homeroom_teacher(p_class_id UUID, p_expected_teacher_id UUID) RETURNS JSONB.
3. Journalisation :
   - Écritures systématiques dans public.school_audit_logs (school_id, actor_id, action, details).

TESTS NÉGATIFS CONÇUS :
--------------------------------------------------------------------------------
1. Test Enseignant d'une Autre École :
   - Tentative d'affecter un teacher_id dont le school_id diffère de la classe.
   - Résultat attendu : Erreur P0012 "Cet enseignant n'appartient pas au même établissement que la classe.".

2. Test Profil Utilisateur non 'teacher' :
   - Tentative d'affecter un compte avec profile_role <> 'teacher'.
   - Résultat attendu : Erreur P0013 "Le profil utilisateur lié à cet enseignant n'a pas le rôle teacher.".

3. Test Enseignant Suspendu ou Inactif :
   - Tentative d'affecter un enseignant avec account_status <> 'active' ou employment_status <> 'active'.
   - Résultat attendu : Erreur P0016 "Cet enseignant ne peut pas être nommé titulaire car son statut n'est pas actif...".

4. Test School Admin d'une Autre École :
   - Tentative d'appel par un admin dont le profile.school_id <> class.school_id.
   - Résultat attendu : Erreur P0008 "Cette classe n'appartient pas à votre établissement.".

5. Test Double Titularisation dans la Même Année :
   - Tentative d'affecter un enseignant déjà titulaire d'une classe A vers une classe B pour la même année scolaire.
   - Résultat attendu : Erreur P0017 "L'enseignant ... est déjà professeur titulaire de la classe ...".

6. Test Concurrence / Titulaire Actuel Inattendu :
   - Tentative de remplacement en passant un p_expected_current_teacher_id obsolète.
   - Résultat attendu : Erreur P0015 "Conflit de concurrence : Le titulaire de la classe a été modifié...".

7. Test Retrait avec Titulaire Attendu Incorrect :
   - Tentative de retrait en passant un p_expected_teacher_id ne correspondant plus à homeroom_teacher_id.
   - Résultat attendu : Erreur P0020 "Conflit de concurrence : Le titulaire actuel de la classe ne correspond plus...".
*/
