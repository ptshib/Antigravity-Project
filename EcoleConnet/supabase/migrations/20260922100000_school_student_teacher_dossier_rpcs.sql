-- Migration SQL : RPCs sécurisées pour la gestion des dossiers individuels Élèves & Enseignants
-- Fichier : supabase/migrations/20260922100000_school_student_teacher_dossier_rpcs.sql
...
BEGIN
    ...
    -- Résumé financier
    SELECT pg_catalog.jsonb_build_object(
        'total_invoiced', pg_catalog.coalesce(SUM(si.total_amount), 0),
        'total_paid', pg_catalog.coalesce(SUM(si.paid_amount), 0),
        'balance_due', pg_catalog.coalesce(SUM(si.balance_amount), 0),
...
BEGIN
    ...
    -- Récupération des affectations aux classes et matières
    SELECT pg_catalog.coalesce(
        pg_catalog.jsonb_agg(
            pg_catalog.jsonb_build_object(
